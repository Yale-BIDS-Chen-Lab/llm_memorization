...
def main(args):
    ...
    print(f"Loaded {len(dataset)} samples from test split")
    ...
    for item in tqdm(dataset, desc="Formatting questions"):
        prompt = format_question(item['question'], item['options'])
        prompts.append(prompt)
        
        # Ground truth is already a letter (A, B, C, D)
        gt = item['answer_idx'].strip().upper()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Evaluate LLM on MedQA-USMLE-4-options dataset')
