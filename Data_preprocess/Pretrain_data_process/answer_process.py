...
# Function to process the dataset with specified input and output word lengths
def process_dataset(sampled_dataset, tokenizer, input_length, output_length, target_sample_size):
    ...
    for i in range(len(sampled_dataset)):
        ...
        clean_text = sampled_dataset[i]['output']
...
def main(args):
    ...
    # Filter the dataset based on the source argument
    ...
    # Double the sample size as a precaution
    initial_sample_size = int(args.sample_size * 10)#medqa and medmcqa use sample_size * 10
    sampled_indices = random.sample(
        range(len(filtered_dataset)), initial_sample_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sample_size', type=int, default=1000,
                        help='Number of samples to retain after processing')
    parser.add_argument('--input_length', type=int, default=50,
                        help='Number of tokens in the input text')
    parser.add_argument('--output_length', type=int, default=500,
                        help='Number of tokens in the output text')
    parser.add_argument('--seed', type=int, default=42,
                        help='Random seed for reproducibility')
    parser.add_argument('--output_file', type=str,
                        default='1k_sampled_data.json', help='Output file to save the results')
    parser.add_argument('--source', type=str, choices=['medmcqa', 'medqa_train', 'pubmedqa.ori_pqaa', 'umls', 'umls_relation'], 
                        required=True, help='Source filter for the dataset')
